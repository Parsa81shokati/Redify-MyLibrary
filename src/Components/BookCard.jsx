...
function BookCard({ book, onOpen }) {
  ...
  return (
    <div className=" relative flex flex-row w-65 sm:w-72 lg:w-75 h-29 sm:h-36 lg:h-48 bg-white border border-none rounded-xl sm:rounded-2xl hover:shadow-md  transition [box-shadow:0_-4px_6px_rgba(0,0,0,0.15)] ">
      <div className=" flex flex-shrink-0 justify-center  overflow-hidden ">
        <img
          src={secureImage}
          alt={book?.title}
          className=" object-cover w-13 sm:w-17 lg:w-26 h-21 sm:h-26 lg:h-40 border border-gray-500 ml-5 mt-4 sm:ml-7 sm:mt-5 lg:ml-4 lg:mt-4"
        />
      </div>

      <div className="flex flex-col gap-1 sm:gap-2 md:gap-3 mt-7 sm:mt-7 lg:mt-10">
        <div className=" flex flex-col items-start px-4 sm:px-5 lg:px-3">
          <h1 className="font-bold text-[12px] sm:text-sm md:text-base lg:text-base/6 line-clamp-2 text-shadow-sm  ">
            {book.title}
          </h1>

          <div className="flex flex-row gap-0.25 sm:gap-0.5 md:gap-1 items-center min-w-0 mt-4">
        ...
        <div className="  absolute flex justify-center items-center w-10  md:w-13 lg:w-14 h-10 sm:h-10 md:h-13 lg:h-14 bg-[#c7e1ff] -bottom-3 -right-3 rounded-tl-lg ">
          <button
            onClick={() => onOpen(book)}
